...
% ---------------------------------------------------------------------
% Acronym popups: every wrapped acronym is BOTH a clickable hyperlink
% to its Glossary entry (works in every PDF viewer, including browsers)
% AND a hover/click popup tooltip via pdfcomment (full support in Adobe
% Acrobat/Reader; other viewers simply show the acronym as a plain link).
% ---------------------------------------------------------------------
% NOTE: the tooltip icon must NOT overlap the hyperlink's own annotation
% rectangle -- two annotations stacked on the same coordinates cause the
% Link annotation to swallow the mouse-hover event in Acrobat, so the
% comment popup never fires even though it exists in the PDF. Instead,
% place the comment icon immediately after the linked text as its own,
% separate, non-overlapping hit target.
\newcommand{\acr}[3]{\hyperlink{gls:#1}{#2}\pdfcomment[icon=Comment,color=codekw,open=false]{#3}}

\newcommand{\AD}{\acr{AD}{AD}{Active Directory: Microsoft's directory service for identity, authentication, and authorization across an organization's users, computers, and groups.}}
\newcommand{\CICD}{\acr{CICD}{CI/CD}{Continuous Integration / Continuous Delivery (or Deployment): automatically building, testing, and releasing code on every change.}}
\newcommand{\CIS}{\acr{CIS}{CIS}{Center for Internet Security: publisher of the CIS Benchmarks, versioned hardening checklists for specific systems.}}
\newcommand{\CVE}{\acr{CVE}{CVE}{Common Vulnerabilities and Exposures: the standard public naming/numbering system for disclosed security vulnerabilities.}}
\newcommand{\IaC}{\acr{IaC}{IaC}{Infrastructure as Code: describing infrastructure in version-controlled text instead of manual console clicks.}}
\newcommand{\IAM}{\acr{IAM}{IAM}{Identity and Access Management: the system for controlling who or what can do what to which resource.}}
\newcommand{\IDP}{\acr{IDP}{IDP}{Internal Developer Platform: the shared infrastructure, pipelines, and tooling a platform team builds for every other team to build on.}}
\newcommand{\KEightS}{\acr{K8s}{K8s}{Kubernetes: a system for running and automatically managing many containers across many machines.}}
\newcommand{\MECM}{\acr{MECM}{MECM/SCCM}{Microsoft Endpoint Configuration Manager (formerly System Center Configuration Manager): tooling for managing on-premises Windows devices at scale.}}
\newcommand{\SARIF}{\acr{SARIF}{SARIF}{Static Analysis Results Interchange Format: a standard format for security scan results so different tools' findings can be viewed in one place.}}
\newcommand{\SAST}{\acr{SAST}{SAST}{Static Application Security Testing: analyzing source code for security-relevant patterns without running it.}}
\newcommand{\SBOM}{\acr{SBOM}{SBOM}{Software Bill of Materials: a complete, machine-readable inventory of every dependency a piece of software uses.}}
\newcommand{\GPO}{\acr{GPO}{GPO}{Group Policy Object: a declared configuration/security setting that Active Directory pushes automatically to computers or users in its scope.}}
\newcommand{\SCA}{\acr{SCA}{SCA}{Software Composition Analysis: checking a dependency inventory against a database of known vulnerabilities.}}
\newcommand{\AWS}{\acr{AWS}{AWS}{Amazon Web Services: the cloud provider with the largest market share, used as this book's primary example.}}
\newcommand{\VPC}{\acr{VPC}{VPC}{Virtual Private Cloud: your own private, isolated network inside a cloud provider's infrastructure.}}
\newcommand{\ECTwo}{\acr{EC2}{EC2}{Elastic Compute Cloud: AWS's core virtual-machine (rentable server) service.}}
\newcommand{\SThree}{\acr{S3}{S3}{Simple Storage Service: AWS's object storage service for files and blobs at effectively unlimited scale.}}
\newcommand{\ECR}{\acr{ECR}{ECR}{Elastic Container Registry: AWS's managed registry for storing and scanning container images.}}
\newcommand{\EKS}{\acr{EKS}{EKS}{Elastic Kubernetes Service: AWS's managed Kubernetes offering.}}
\newcommand{\KMS}{\acr{KMS}{KMS}{Key Management Service: AWS's managed service for creating and rotating encryption keys.}}
\newcommand{\HCL}{\acr{HCL}{HCL}{HashiCorp Configuration Language: the language Terraform configuration files are written in.}}
\newcommand{\GPG}{\acr{GPG}{GPG}{GNU Privacy Guard: an implementation of the OpenPGP standard, commonly used to cryptographically sign git commits.}}
\newcommand{\TLS}{\acr{TLS}{TLS}{Transport Layer Security: the cryptographic protocol that encrypts network traffic in transit (the successor to SSL).}}
\newcommand{\DNS}{\acr{DNS}{DNS}{Domain Name System: translates human-readable hostnames into IP addresses.}}
\newcommand{\SSH}{\acr{SSH}{SSH}{Secure Shell: an encrypted protocol for remote login and secure command execution, also used for signing git commits and authenticating to git remotes.}}
\newcommand{\API}{\acr{API}{API}{Application Programming Interface: a defined way for one piece of software to request functionality or data from another.}}
\newcommand{\VM}{\acr{VM}{VM}{Virtual Machine: a full virtualized computer, including its own kernel, distinct from the lighter-weight isolation a container provides.}}
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Cloud basics} --- not "AWS trivia," but the five or six concepts (compute,
    ...
    description.
  \item \IaC{} (\textbf{Infrastructure as Code}) --- Terraform, and more importantly the concept of
...
\begin{enumerate}[leftmargin=2em]
  \item \textbf{Compute} --- a place your code actually runs: a \VM{} (\AWS{} \ECTwo{}),
  ...
  \item \textbf{Storage} --- durable places to keep data: object storage for files and
    blobs (\AWS{} \SThree{}), block storage attached to a single virtual machine like a hard disk
...
\IaC{} (\textbf{Infrastructure as Code}) means describing the infrastructure you want in a text
...
by hand does not scale past a very small team. \textbf{Kubernetes} (often \KEightS{}) is a
...
\CICD{} (\textbf{Continuous Integration (CI)}) is the practice of automatically building and testing
...
\SAST{} analyzes source code for security-relevant patterns \emph{without running it}: building a
...
A \SBOM{} (\textbf{Software Bill of Materials}) is a complete, machine-readable inventory of
every library your software depends on and its exact version -- an ingredient list.
\textbf{Dependency scanning} (or \SCA{} (Software Composition Analysis)) checks that
inventory against a database of known, publicly disclosed vulnerabilities (\CVE{}s -- Common
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Centralize findings.} Several scanners can emit a standard format called
    \SARIF{} (\textbf{Static Analysis} Results Interchange Format); uploading all of them to
...
\AD{} (\textbf{Active Directory}) is Microsoft's directory service: a database of every user,
...
A \GPO{} (\textbf{Group Policy Object}) is a declared set of configuration settings --
...
\MECM{} (\textbf{Microsoft Endpoint Configuration Manager}, the modern name for what was long
...
The \CIS{} (\textbf{Center for Internet Security}) publishes detailed, versioned, freely
...
\begin{note}[How to use the acronym links in this book]
Every acronym below that appears underlined and colored the first time it is introduced
in the body text (for example \AD{} or \IAM{}) is a live PDF link: click it to jump here,
and click ``Return'' (or use your viewer's back button) to jump back to where you were
reading. Immediately after each linked acronym is a small comment/note icon
(\raisebox{-0.1em}{\pdfcomment[icon=Comment,color=codekw,open=false]{example}}); in Adobe
Acrobat or Adobe Reader specifically, hovering over \emph{that icon} (not the link text
itself) previews the full definition inline, and clicking it pins the popup open -- the
icon is deliberately a separate click target from the link, since two overlapping
annotations at the same coordinates would cause the link to swallow the hover event
before the comment popup could ever fire. Other PDF viewers (browsers, Preview, most
mobile readers) do not render that icon or its popup at all, and only the link will work.
\end{note}

\begin{description}[leftmargin=2.2em,style=nextline]
\item[\hypertarget{gls:AD}{AD (Active Directory)}] Microsoft's directory service for identity, authentication,
  and authorization across an organization's users, computers, and groups.
\item[\hypertarget{gls:API}{API}] Application Programming Interface: a defined way for one piece of software to
  request functionality or data from another.
\item[\hypertarget{gls:AWS}{AWS}] Amazon Web Services: the cloud provider with the largest market share, used
  as this book's primary example.
\item[\hypertarget{gls:CICD}{CI/CD}] Continuous Integration / Continuous Delivery (or Deployment): automatically
  building, testing, and (for Deployment) releasing code on every change.
\item[\hypertarget{gls:CIS}{CIS Benchmark}] An industry-standard, detailed hardening checklist for a specific
...
\item[\hypertarget{gls:CVE}{CVE}] Common Vulnerabilities and Exposures: the standard public naming/numbering
...
\item[\hypertarget{gls:DNS}{DNS}] Domain Name System: translates human-readable hostnames into IP addresses.
\item[Docker] The most common tool for building and running containers.
\item[\hypertarget{gls:ECR}{ECR}] Elastic Container Registry: AWS's managed registry for storing and scanning
  container images.
\item[\hypertarget{gls:ECTwo}{EC2}] Elastic Compute Cloud: AWS's core virtual-machine (rentable server) service.
\item[\hypertarget{gls:EKS}{EKS}] Elastic Kubernetes Service: AWS's managed Kubernetes offering.
\item[\hypertarget{gls:GPG}{GPG}] GNU Privacy Guard: an implementation of the OpenPGP standard, commonly used
  to cryptographically sign git commits.
\item[\hypertarget{gls:GPO}{GPO (Group Policy Object)}] A declared configuration/security setting that Active
  Directory pushes automatically to computers or users in its scope.
\item[\hypertarget{gls:HCL}{HCL}] HashiCorp Configuration Language: the language Terraform configuration
  files are written in.
\item[\hypertarget{gls:IaC}{IaC (Infrastructure as Code)}] Describing infrastructure in version-controlled text
  files instead of manual cloud-console clicks, so it is repeatable, reviewable, and
  recreatable.
\item[\hypertarget{gls:IAM}{IAM (Identity and Access Management)}] The system for controlling who or what can do
  what to which resource.
\item[\hypertarget{gls:IDP}{IDP (Internal Developer Platform)}] The shared infrastructure, pipelines, and tooling
  a platform team builds for every other team to build on.
\item[\hypertarget{gls:K8s}{Kubernetes (K8s)}] A system for running and automatically managing many containers
  across many machines.
\item[\hypertarget{gls:KMS}{KMS}] Key Management Service: AWS's managed service for creating and rotating
  encryption keys.
...
\item[\hypertarget{gls:MECM}{MECM/SCCM}] Microsoft Endpoint Configuration Manager (formerly System Center
...
\item[\hypertarget{gls:SThree}{S3}] Simple Storage Service: AWS's object storage service for files and blobs
  at effectively unlimited scale.
\item[\hypertarget{gls:SARIF}{SARIF}] Static Analysis Results Interchange Format: a standard format for security
  scan results, so different tools' findings can be viewed in one place.
\item[\hypertarget{gls:SAST}{SAST}] Static Application Security Testing: analyzing source code for
  security-relevant patterns without running it.
\item[\hypertarget{gls:SBOM}{SBOM}] Software Bill of Materials: a complete, machine-readable inventory of every
  dependency a piece of software uses.
\item[\hypertarget{gls:SCA}{SCA (Software Composition Analysis)}] Checking a dependency inventory against a
  database of known, publicly disclosed vulnerabilities; see also \SBOM{}.
...
\item[\hypertarget{gls:SSH}{SSH}] Secure Shell: an encrypted protocol for remote login and secure command
  execution, also used for signing git commits and authenticating to git remotes.
...
\item[\hypertarget{gls:TLS}{TLS}] Transport Layer Security: the cryptographic protocol that encrypts network
  traffic in transit (the successor to SSL).
\item[\hypertarget{gls:VM}{VM}] Virtual Machine: a full virtualized computer, including its own kernel,
  distinct from the lighter-weight isolation a container provides.
\item[\hypertarget{gls:VPC}{VPC}] Virtual Private Cloud: your own private, isolated network inside a cloud
