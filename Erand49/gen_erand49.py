...
<h2>Frame — CAD-projected views (build123d)</h2>
<p class="sub">The frame now has a parametric solid model — <code>frame_cad.py</code> → <code>frame.step</code> —
built from the same DXF parameters and the hand-edited neck curves. These views are projected from the
solid with true hidden-line removal (dashed = hidden), so intersections (the pillar's elliptical pass
...
<td style="width:45%"><b>Neck stack at the shoulder lap (CAD slice)</b><br/><img src="cad-sec-neckstack.svg" style="max-width:60%"/></td>
</tr></table>
...
weld callouts until annotation is migrated.</p>
...
<line x1="266.5" y1="269" x2="373.5" y2="331" stroke="#c58a1f" stroke-width="1.2" stroke-dasharray="5 4"/>
