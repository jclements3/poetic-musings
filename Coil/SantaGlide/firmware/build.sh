#!/usr/bin/env bash
# build.sh — synthesize the Santa Glide bitstream for the Alchitry Cu.
#
...
SRC=verilog/SantaGlide.topEntity/santa_glide.v
OUT=build
...
"$TOOLS/yosys" -q -l "$OUT/yosys.log" \
  -p "read_verilog $SRC cu_top.v; synth_ice40 -top cu_top -json $OUT/santa_glide.json"
...
"$TOOLS/nextpnr-ice40" --hx8k --package cb132 --freq 50 \
  --json "$OUT/santa_glide.json" --pcf santa_glide.pcf \
  --asc "$OUT/santa_glide.asc" --log "$OUT/nextpnr.log"

# 3. Bitstream.
"$TOOLS/icepack" "$OUT/santa_glide.asc" "$OUT/santa_glide.bin"

echo
grep -E "Max frequency for clock" "$OUT/nextpnr.log" | tail -1
echo "bitstream: $OUT/santa_glide.bin"
