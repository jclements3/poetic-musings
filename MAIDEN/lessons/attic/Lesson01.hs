...
--
-- Note what is absent: no port list, no process, no sensitivity list. Combinational logic is an
-- expression, so the classic Verilog bug of omitting a signal from the sensitivity list has no
-- way to exist here. The width `8` is written in the type and nowhere else.
...
-- demands Unsigned 4 out; x is Unsigned 8; there is no consistent choice, so compilation stops.
--
-- Verilog accepts the equivalent without comment:
--
--     assign result = x + 1;     // result 4-bit, x 8-bit
--
-- It elaborates, synthesises, and silently discards the top four bits. The failure surfaces on
-- the bench when a value above 15 wraps.
--
...
-- the carry), but the top level connects it to a signal declared at EDGE_POSITION_BITS. The MSB
-- is dropped and an overflowing pulse-centre sum wraps. Nothing in the Verilog toolchain
...
--
-- `diff` against (2) is empty -- resize on an Unsigned narrowing IS truncation. It differs from
-- truncateB only in that it also works in the widening direction (zero-extend for Unsigned,
-- sign-extend for Signed), so it is the one to reach for in parameterised code.
...
--
-- CAUTION, and worth breaking on purpose now rather than in the Doppler chain later: `resize x`
-- narrows FIRST, then satSucc increments with clamping. Input 20 truncates to 4, then
-- increments to 5 -- not 15. The saturation protects the increment, not the narrowing. To
-- clamp the narrowing itself you must compare at the wide width before resizing.
--
...
------------------------------------------------------------------------------------------------
-- Exercise, if you want it: write a version that clamps the NARROWING correctly -- input 20
-- should give 15, not 5.
------------------------------------------------------------------------------------------------
